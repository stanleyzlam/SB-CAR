...
### Plot Heatmap ----
setwd(paste0(ROOT_DIRECTORY, "/figures/functionalheatmap/unique") )
pltdf <- read.table("AggregatedInsertionsByFunctionalRegion.txt", header = T, sep = "\t")

pltdf <- pltdf[,c(1,2,3,4,5,6,7,11)]
pltdf <- pltdf[c(6,5,7,3,2,1,8,9,4),] # reorder row names to c("Genes", "Exons", "Introns", "Coding exons", "5'UTR", "3'UTR", "Upstream 10kb", "Upstream 1kb", "Cancer genes")
