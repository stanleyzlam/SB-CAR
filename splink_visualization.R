...
ROOT_DIRECTORY = 'C:/Users/stanl/OneDrive/Chen Lab/2021-2022 Gap Year 1/SB-CAR/github_upload/files-for-visualization'
...
generate_gr_AAV_mRNA <- function(keep_redundant = F){
  
  setwd(paste0(ROOT_DIRECTORY, "/Fastq/bed") )
  files <- list.files()
  ...
  for (i in 1:length(files)){
    ...
    #bed.gr <- bed.gr[which(table(bed.gr)>=1)]
...
### MC+mRNA ----
generate_gr_MC_mRNA <- function(keep_redundant = F){
...
### AAVonly ----
generate_gr_AAV_only <- function(keep_redundant = F){
...
grs <- list(gr_random1e6, gr_AAV_mRNA, gr_MC_mRNA, gr_Querques)
...
pltdf <- read.table("figures/functionalheatmap/unique/AggregatedInsertionsByFunctionalRegion.txt", header = T, sep = "\t")

pltdf <- pltdf[,c("AAV.mRNA.1","AAV.mRNA.2","AAV.mRNA.3","MC.mRNA.1","MC.mRNA.2","MC.mRNA.3","Querques.HIV", "random1e6")]
